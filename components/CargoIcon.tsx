...
const CargoIcon: React.FC<CargoIconProps> = ({ type, className = "w-4 h-4" }) => {
  const normalizedType = type.toLowerCase();

  if (normalizedType.includes('perishable')) {
    // Snowflake
    return (
  ...
  if (normalizedType.includes('hazard')) {
    // Shield Exclamation / Warning
    return (
      <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  if (normalizedType.includes('machinery')) {
  ...
  if (normalizedType.includes('electronics')) {
    // Chip
    return (
      <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  if (normalizedType.includes('textile')) {
    // Fabric/Roll
    return (
      <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
